...
const SectionListScreen = () => {
  return (
    <ThemedView margin>
      <ThemeCard>
        <SectionList sections={houses} keyExtractor={(item) => item}  renderItem={({item}) => <ThemedText>{item}</ThemedText>} ListHeaderComponent={() => (
          <ThemedText type='h1' className='font-bold mb-3'>Personajes</ThemedText>
        )}
        renderSectionHeader={({section}) => <ThemedText type='h1' className='bg-light-background dark:bg-dark-background p-2 rounded'>{section.title}</ThemedText>}
        stickySectionHeadersEnabled
        ListFooterComponent={() => (
          <ThemedText type='h1' className='font-bold bg-light-background dark:bg-dark-background p-2 rounded mb-10'>Secciones {houses.length}</ThemedText>
        )}
